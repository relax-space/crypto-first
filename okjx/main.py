...
class CryptoR:
    ...
    def __init__(self, key: str, mode: int, iv: str):
        _iv = iv.encode("utf8") if iv else None
        _key = key.encode("utf8")
        self.aes = AES.new(_key, mode, _iv)
...
def call_en():
    ...
    iv = "057f1eed099f2f7e"
    aes = CryptoR(key, AES.MODE_CBC, iv)
    e1 = aes.encrypto(text)
    e2 = crypto_js("encrypt2", text, key, iv)
...
def call_de():
    ...
    d2 = crypto_js("decrypt2", text, key, iv)
